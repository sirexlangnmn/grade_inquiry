 <!--Sort Grade Modal-->
<div class="modal fade" id="add_grade_advance" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
/*	$subject_query1  = mysqli_query($con, "SELECT * FROM subject WHERE subject_id = '$subject_id")
	or die(mysqli_error());
	$subject_row1 = mysqli_fetch_array($subject_query1);
	$unit =  $subject_row1['unit'];
	
	
	$grade_query1 = mysqli_query($con, "SELECT SUM(unit) FROM grade WHERE student_id = '$student_id' AND level_id = '$level_id' AND semester = '$semester'")or die(mysqli_error());
	$grade_rows = mysqli_fetch_array($grade_query1);
	$units = $grade_row1['SUM(unit)'];
